...
def fetch_elmo() -> Response:  # pylint: disable=unused-variable
    ...
    dimension = req.get('dimension', 1024)
    result = []
    try:
        logger.info("Determining ELMo vectors for %s sentences %s" %
                    (len(sentences), str(sentences)[:1000]))
        ...
        for (sentence, tokens, embedding) in zip(sentences, tokenized,
                                                 embedded):
            vectors = project(dimension, embedding)
            result += [{
                "sentence": sentence,
                "tokens": tokens,
...
def project(dimension, embedding):
